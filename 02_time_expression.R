...
### subset columnmeta tot contain only NCX samples 
...
NCX <- NCX_matrix["ENSG00000101040",] %>%
  as.data.frame() %>% 
...
OR <- OtherRegions_matrix["ENSG00000101040",] %>%
  as.data.frame() %>% 
  rownames_to_column("geneID") %>%
  melt() %>% 
  cbind(OtherRegions_samples) %>%  
  ggplot(aes(stage, value))  + 
  geom_boxplot(aes(fill=stage), alpha=0.65)  + 
  geom_jitter(size=2.5) +
  scale_color_viridis(discrete = TRUE, option = "magma")+
  scale_fill_viridis(discrete = TRUE, direction = 1)  + theme_bw() +
  ylab("RPKM") + xlab("Stage") +
  theme(legend.position = "none", 
        axis.text=element_text(size=11, face="bold", color="black"), 
        axis.title = element_text(siz=12, face="bold", color="black"), 
        axis.text.x = element_text(angle=90)) +
